document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Локальное хранилище, для сохранения заказов, даже после закрытия страницы*/
  const toStorage = () => {
    localStorage.setItem('freeOrders', JSON.stringify(orders));
  };
  /* Возвращаем готовую строчку с числом и количеством */
  const declOfNum = (number, titles) => number + ' ' + titles[(number % 100 > 4 && number % 100 < 20) ?
    2 : [2, 0, 1, 1, 1, 2][(number % 10 < 5) ? number % 10 : 5]];

  /* Посчитать сколько дней осталось до дедлайна заказа */
